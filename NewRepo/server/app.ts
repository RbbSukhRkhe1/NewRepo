...
export function createApp() {
  ...
  // Admin-only controls: soft-delete (archive) and revert a cause.
  api.get(
    '/causes/admin',
    requireAuth,
    requireAdmin,
    (_req: express.Request, res: express.Response) => {
      const rows = db
        .prepare(
          `SELECT id, title, description, goal_eth, raised_eth, active, created_at FROM causes ORDER BY id DESC`
        )
        .all() as {
        id: number;
        title: string;
  ...
  api.post(
    ...
    (req: express.Request, res: express.Response) => {
      ...
      const r = db.prepare('UPDATE causes SET active = 0 WHERE id = ?').run(id);
      if (r.changes === 0) return res.status(404).json({ error: 'Cause not found' });
      res.json({ ok: true });
    }
  );
  ...
  api.post(
    ...
    (req: express.Request, res: express.Response) => {
      const id = Number(req.params.id);
      if (!Number.isFinite(id)) return res.status(400).json({ error: 'Invalid id' });
      const r = db.prepare('UPDATE causes SET active = 1 WHERE id = ?').run(id);
      if (r.changes === 0) return res.status(404).json({ error: 'Cause not found' });
      res.json({ ok: true });
    }
  );
  ...
  api.get('/ledger', (_req, res) => {
    ...
    const mapped = rows.map((r) => {
      ...
      const fromAddr = r.from_addr.toLowerCase();
      const toAddr = r.to_addr.toLowerCase();
      const fromName =
        fromAddr === vl ? vaultName : r.from_display_name ?? maskAddr(r.from_addr);
      const toName =
        toAddr === vl ? vaultName : r.to_display_name ?? maskAddr(r.to_addr);
