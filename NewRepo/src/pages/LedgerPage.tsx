...
export function LedgerPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 text-left sm:py-16">
      <p className="font-display text-xs font-semibold uppercase tracking-[0.28em]" style={{ color: ACCENT }}>
        Public record
      </p>
      <h1 className="font-display mt-3 text-3xl font-bold text-white sm:text-4xl">Transparency ledger</h1>
      <p className="mt-3 max-w-lg text-sm leading-relaxed text-zinc-500">
        A living feed of donations and payouts — so anyone can see how value moves from donors to the
        vault and out to partners. Refreshes automatically every few seconds.
      </p>

      <div
        className="mt-6 reveal rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl"
        style={{ boxShadow: `0 0 0 1px ${ACCENT}14 inset` }}
        data-reveal
      >
        <p className="text-xs font-medium uppercase tracking-widest text-zinc-500">Totals · this session</p>
      ...
      <div
        className="mt-6 reveal overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.03] shadow-2xl backdrop-blur-xl"
        style={{ boxShadow: `0 25px 80px -20px #000, 0 0 0 1px ${ACCENT}18 inset` }}
        data-reveal
      >
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/[0.06] px-4 py-4 sm:px-6">
          ...
          <nav className="flex rounded-full border border-white/10 bg-black/30 p-1 text-xs font-medium">
            {(
              ...
              <button
                ...
                style={tab === key ? { backgroundColor: ACCENT, color: '#041214' } : undefined}
              >
                {label}
              </button>
            ))}
        ...
        >
          {filtered.length === 0 && !loadError ? (
            <div className="px-6 py-16 text-center text-sm text-zinc-500">
              No activity yet. When donations or payouts happen, they&apos;ll show up here. Start the app
              and chain locally if you&apos;re testing.
            </div>
                    ...
                    <div className="flex gap-4">
                      <div
                        ...
                        style={{
                          ...
                          background: `linear-gradient(145deg, ${ACCENT}22, rgba(34,197,94,0.12))`,
                          color: ACCENT,
                        }}
                      >
